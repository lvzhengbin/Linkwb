...
import com.ice.wb.widget.utils.CalculationUtil;
...
public class SquareProgressView extends View {
    ...
    public DrawStop getDrawEnd(float percent, Canvas canvas) {
        ...
        strokewidth = CalculationUtil.convertDpToPx(widthInDp, getContext());
        if (percent > 0 && percent <= canvas.getWidth()){
            // top
            drawStop.place = Place.TOP;
            drawStop.location = percent;
        }else {
            if (percent > canvas.getWidth()){
                float second = percent - canvas.getWidth();
                if (second > (canvas.getHeight() - strokewidth)){
                    ...
                    if (third > (canvas.getWidth() - strokewidth)){
                        ...
                        drawStop.location = canvas.getHeight() - strokewidth - forth - strokewidth;
                    }else {
                        //bottom
